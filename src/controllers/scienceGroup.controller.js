...
export async function createScienceGroupController(req, res, next) {
    try {
        const result = await createScienceGroup(req.validated ?? req.body);
...
export async function updateScienceGroupController(req, res, next) {
    try {
        const result = await updateScienceGroup(req.params.id, req.validated ?? req.body);
        res.status(200).json({ success: true, data: result });
    } catch (err) { next(err); }
}
...
export async function deleteScienceGroupController(req, res, next) {
    try {
        ...
        res.status(200).json({ success: true, message: "Kelompok keilmuan berhasil dihapus" });
    } catch (err) { next(err); }
}
