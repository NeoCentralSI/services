...
import * as yudisiumService from "../services/yudisium/core.service.js";
...
export const finalizeRegistration = async (req, res, next) => {
  ...
  try {
    ...
    const data = await yudisiumService.finalizeRegistration(req.params.id, req.user?.id);
...
export const streamRequirementFile = async (req, res, next) => {
  try {
    const { id: yudisiumId, participantId, itemId } = req.params;
    const fileInfo = await participantService.getRequirementFilePath(
      yudisiumId,
      participantId,
      itemId,
      req.user
