...
import { finalizeParticipants } from "./participant.service.js";
...
const formatYudisium = (item) => ({
  ...
  decreeFilePath: item.decreeFilePath ?? null,

  decreeFileName: item.decreeFileName ?? null,

  decreeMimeType: item.decreeMimeType ?? null,

  decreeFileSize: item.decreeFileSize ?? null,

  decreeUploadedAt: item.decreeUploadedAt ?? null,

  decreeUploadedBy: item.decreeUploadedBy ?? null,

  decreeDocument: item.decreeFilePath
    ? {
        id: item.id,
        fileName: item.decreeFileName ?? 'SK-Yudisium.pdf',
        filePath: item.decreeFilePath,
      }
    : null,
...
const isFinalizedOrAppointed = (item) => !!item.appointedAt || !!item.decreeFilePath;
...
const finalizeYudisiumResults = async (yudisiumId, db = prisma) => {

  try {

    const participants = await db.yudisiumParticipant.findMany({
    ...
    await db.yudisiumParticipant.updateMany({
    ...
    if (studentIds.length > 0) {
      ...
      await db.studentCplScore.updateMany({
...
export const getRepository = async (search) => {
  ...
  const participantRequirements = await prisma.yudisiumParticipantRequirement.findMany({
    ...
    where: { yudisiumRequirementItemId: { in: itemIds }, status: "approved" },
  ...
  const thesisIds = [...new Set(participants.map((p) => p.thesisId).filter(Boolean))];
  ...
  // Step 6: users + topics in parallel
  ...
  const enrichedDocs = filteredPR.map((pr) => {
    ...
    const thesis = thesisMap[participant?.thesisId];

    const student = userMap[thesis?.studentId];

    const topic = topicMap[thesis?.thesisTopicId];
...
const enrichYudisium = async (item) => {

  if (!item) return null;

  const responseCount = await prisma.yudisiumParticipant.count({

    where: { yudisiumId: item.id, exitSurveySubmittedAt: { not: null } },

  });

  return formatYudisium({ ...item, responseCount });
...
export const getYudisiumList = async () => {
  ...
  return await Promise.all(data.map(enrichYudisium));
...
export const getYudisiumDetail = async (id) => {
  ...
  return await enrichYudisium(data);
...
export const createYudisium = async (data) => {
  ...
  if (decreeFile) {
    ...
    const updated = await repository.update(created.id, {
      ...
      decreeFilePath: relPath,

      decreeFileName: decreeFile.originalname,

      decreeMimeType: decreeFile.mimetype || "application/pdf",

      decreeFileSize: decreeFile.size,
...
export const updateYudisium = async (id, data) => {
  ...
  if (decreeFile) {
    if (!existing.appointedAt) {
      throwError("SK yudisium hanya dapat diunggah setelah peserta ditetapkan", 422);
    }
    if (deriveDisplayStatus(existing) !== "completed") {
      throwError("SK yudisium hanya dapat diunggah setelah periode yudisium selesai (setelah tanggal pelaksanaan)", 422);
    }
  }
  ...
  if (decreeFile) {
    ...
    updateData.decreeFilePath = relPath;

    updateData.decreeFileName = decreeFile.originalname;

    updateData.decreeMimeType = decreeFile.mimetype || "application/pdf";

    updateData.decreeFileSize = decreeFile.size;

    updateData.decreeUploadedBy = userId || null;
    ...
    try {
      ...
      const updated = await prisma.$transaction(async (tx) => {
        ...
        const res = await repository.update(id, updateData, tx);

        await finalizeYudisiumResults(id, tx);
...
export const finalizeRegistration = async (yudisiumId, userId = null) => {

  return await finalizeParticipants(yudisiumId, userId);
