...
const guidanceRepo = bimbinganRepository;

async function getActiveYear() {
    const activeYear = await pendaftaranRepository.getActiveAcademicYear();
    if (!activeYear) {
        const error = new Error("Tidak ada tahun akademik aktif.");
...
/**
 * Save lecturer notes for the internship report draft.
 * @param {string} lecturerId 
 * @param {string} internshipId 
 * @param {Object} data - { notes, feedbackFile }
 * @returns {Promise<Object>}
 */
export async function verifyFinalReport(lecturerId, internshipId, { status, notes, feedbackFile }) {
    if (!["APPROVED", "REVISION_NEEDED"].includes(status)) {
        const error = new Error("Status verifikasi laporan akhir tidak valid.");
        error.statusCode = 400;
        throw error;
    }

    // Verify internship belongs to this lecturer
    const internship = await bimbinganRepository.findSupervisedInternshipById(internshipId, lecturerId);
    if (!internship) {
    ...
    if (!internship.reportDocumentId) {
        const error = new Error("Laporan akhir belum diunggah.");
        error.statusCode = 400;
        throw error;
    }
    ...
    try {
        ...
        const title = isApproved ? `Laporan Akhir Disetujui` : `Revisi Laporan Akhir`;
        let message = isApproved
            ? `Dosen Pembimbing telah menyetujui Laporan Akhir Anda sebagai laporan final.`
            : `Dosen Pembimbing meminta revisi Laporan Akhir Anda.`;
        if (notes) {
