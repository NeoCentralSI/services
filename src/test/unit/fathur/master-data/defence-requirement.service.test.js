...
const requirementOne = {
  ...
  name: "Buku Tugas Akhir",
  description: "Buku final",
  displayOrder: 1,
};
...
describe("Defence Requirement Service", () => {
  ...
  it("creates a trimmed requirement at the next display order", async () => {
    ...
    mockPrisma.thesisDefenceRequirement.create.mockResolvedValue({ id: "new-id" });
  ...
  it("allows description updates after documents have been uploaded", async () => {
    mockPrisma.thesisDefenceRequirement.findUnique.mockResolvedValue(withCount(requirementOne, 1));
  ...
  it("locks the name after a related document exists", async () => {
    ...
    expect(mockPrisma.thesisDefenceRequirement.update).not.toHaveBeenCalled();
  });

  it("deletes an unused requirement", async () => {
    mockPrisma.thesisDefenceRequirement.findUnique.mockResolvedValue(withCount(requirementOne));
    mockPrisma.thesisDefenceRequirement.delete.mockResolvedValue(requirementOne);

    await service.remove(requirementOne.id);

    expect(mockPrisma.thesisDefenceRequirement.delete).toHaveBeenCalledWith({
      where: { id: requirementOne.id },
    });
  });
  ...
  it("copies only new-schema fields into an empty target year", async () => {
    ...
    mockPrisma.thesisDefenceRequirement.findMany
      .mockResolvedValueOnce([requirementOne])
      .mockResolvedValueOnce([copied]);
    mockPrisma.thesisDefenceRequirement.count.mockResolvedValue(0);
    mockPrisma.thesisDefenceRequirement.createMany.mockResolvedValue({ count: 1 });
    ...
    expect(mockPrisma.thesisDefenceRequirement.createMany).toHaveBeenCalledWith({
      data: [{
  ...
  it("refuses to overwrite requirements in the target year", async () => {
    mockPrisma.thesisDefenceRequirement.findMany.mockResolvedValue([requirementOne]);
    mockPrisma.thesisDefenceRequirement.count.mockResolvedValue(1);
