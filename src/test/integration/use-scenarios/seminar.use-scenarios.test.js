...
describe.sequential("Use Scenario Integration — Thesis Seminar (UC-11 s.d. UC-17)", () => {
  ...
  beforeAll(async () => {
    ...
    originalMinimumScore = academicYear.thesisSeminarMinimumScore;
    if (originalMinimumScore == null) {
      await prisma.academicYear.update({ where: { id: academicYear.id }, data: { thesisSeminarMinimumScore: 55 } });
    }
    ...
    thesisCpmk = await prisma.thesisCpmk.create({
      data: { academicYearId: academicYear.id, code: "CPMK-" + marker, description: "CPMK skenario seminar" },
    });
    assessmentCriterion = await prisma.thesisSeminarAssessmentCriteria.create({
      data: { thesisCpmkId: thesisCpmk.id, name: "Kriteria seminar " + marker, maxScore: 100, displayOrder: 999 },
    });
  ...
  afterAll(async () => {
    ...
    await prisma.thesisSeminarAssessmentCriteria.deleteMany({ where: { id: assessmentCriterion?.id } }).catch(() => {});
    await prisma.thesisCpmk.deleteMany({ where: { id: thesisCpmk?.id } }).catch(() => {});
    if (academicYear && originalMinimumScore == null) await prisma.academicYear.update({ where: { id: academicYear.id }, data: { thesisSeminarMinimumScore: null } }).catch(() => {});
